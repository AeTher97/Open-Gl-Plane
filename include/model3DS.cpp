...
model3DS::model3DS(const char *filename, const float scale, bool stereo) : m_filename(filename), m_scale(scale),
                                                                           m_stereo(stereo) {
...
bool model3DS::readChunk(std::ifstream *modelFile, int objectStart, int objectLength) {
    ...
    while ((modelFile->tellg() < (objectStart + objectLength)) && !modelFile->eof()) {
        ...
        if (DEBUG_OUTPUT)
            std::cout << std::hex << "[" << offset << "] chunk: 0x" << chunkHeader << " (" << offset << "-"
                      << (offset + chunkLength) << ")" << std::dec << std::endl;
        switch (chunkHeader) {
            ...
            case CHUNK_MATERIAL_NAME:
                ...
                if (DEBUG_OUTPUT)
                    std::cout << "  Material: " << m_currentMaterial->getName() << "("
                              << m_currentMaterial->getName().size() << ")" << std::endl;
                name.erase();
                break;
...
void mesh3DS::calculateNormals() {
    ...
    for (int face = 0; face < int(m_faces.size()); face += 3) {
        ...
        vtx1.set(m_vertices[m_faces[face] * 3], m_vertices[(m_faces[face] * 3) + 1],
                 m_vertices[(m_faces[face] * 3) + 2]);
        vtx2.set(m_vertices[m_faces[face + 1] * 3], m_vertices[(m_faces[face + 1] * 3) + 1],
                 m_vertices[(m_faces[face + 1] * 3) + 2]);
        vtx3.set(m_vertices[m_faces[face + 2] * 3], m_vertices[(m_faces[face + 2] * 3) + 1],
                 m_vertices[(m_faces[face + 2] * 3) + 2]);
...
double mesh3DS::calculateSpecialTransformY(int vertexIndex) {
    switch (m_specialTransform) {
        case 1:
            return m_specialTransformTable[
                    ((int) (m_specialTransformTick / 3) + (int) (abs(m_vertices[vertexIndex]) * 256) +
                     (int) (abs(m_vertices[vertexIndex + 2]) * 256)) % 314];
        case 2:
            return (m_specialTransformTable[((int) (m_specialTransformTick / 5)) % 314] * 80) /
                   ((float) 20 + abs(m_vertices[vertexIndex + 2]));
    }
    return 0;
}
    ...
    if (!pregenetateList && !m_specialTransform)
        ...
        for (int window = 1; window < (m_parentModel->getStereo() ? 3 : 2); window++) {
            ...
            if (!m_specialTransform) {
                ...
                glNewList(window == 1 ? getListLeft() : getListRight(),
                          GL_COMPILE); //definicja zawarto?ci listy("cube")
            }
            ...
            for (materialsIter = m_materialFaces.begin(); materialsIter != m_materialFaces.end(); ++materialsIter) {
                ...
                float adjustedSpecular[4] = {specular[0] * shininess, specular[1] * shininess, specular[2] * shininess,
                                             1};
                ...
                switch (m_drawMode) {
                    case DRAW_IMMEDIATE_MODE:
                        ...
                        for (face = 0; face < numFaces; face += 3) {
                            ...
                            glVertex3f(m_vertices[vertexIndex] + calculateSpecialTransformX(vertexIndex),
                                       m_vertices[vertexIndex + 1] + calculateSpecialTransformY(vertexIndex),
                                       m_vertices[vertexIndex + 2] + calculateSpecialTransformZ(vertexIndex));
                            if (hasTextureMap) {
...
// - int: id tekstury: jeœli wiêksze od 0 to wymusa automatyczne teksturowanie t¹ tekstur¹, jeœli 0 lub brak - przyjmuje teksturowanie z pliku
// - bool: centrowanie obiektu: jeœli true lub brak: za pozycjê obiektu przymuje jego geometryczny œrodek, jeœli false - przyjmje wspó³rzêdne z pliku
void model3DS::draw(int TextureID, bool centruj) {
    ...
    if (texGenS == GL_TRUE) glEnable(GL_TEXTURE_GEN_S);
        ...
        glDisable(GL_TEXTURE_GEN_S); // ustawienie poprzednich wartoœci
