...
textureBMP::textureBMP(const char *filename, const int textureId, const bool stereoMode) {
    ...
    if (m_bpp != 32 && m_bpp != 24) {
        std::cout << "[BMP] ERROR: Invalid texture color depth, '" << filename << "' must be uncompressed 24/32bpp BMP"
                  << std::endl;
        UnmapViewOfFile(fileData);
        CloseHandle(hTextureFileMapping);
        CloseHandle(hTextureFile);
    ...
    switch (m_bpp) {
        ...
        case 32:
            ...
            break;
        default:
            std::cout << "[BMP] ERROR: Invalid texture color depth, '" << filename
                      << "' must be uncompressed 24/32bpp BMP" << std::endl;
            UnmapViewOfFile(fileData);
            CloseHandle(hTextureFileMapping);
            CloseHandle(hTextureFile);
            return;
