...
textureTGA::textureTGA(const char *filename, const int textureId, const bool stereoMode) {
    ...
    // Open file
    HANDLE hTextureFile = CreateFile(filename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    if (colorMapType == 1 || imageType != 2) {
        std::cout << "[TGA] ERROR: '" << filename
                  << "' is an texture invalid format\n[TGA] ERROR: It should be an uncompressed 24/32bpp TGA"
                  << std::endl;
    ...
    if (m_bpp != 32 && m_bpp != 24) {
        std::cout << "[TGA] ERROR: Invalid texture color depth, '" << filename << "' must be uncompressed 24/32bpp TGA"
                  << std::endl;
        UnmapViewOfFile(fileData);
        CloseHandle(hTextureFileMapping);
        CloseHandle(hTextureFile);
    ...
    switch (m_bpp) {
        ...
        case 32:
            ...
            break;
        default:
            std::cout << "[TGA] ERROR: Invalid texture color depth, '" << filename
                      << "' must be uncompressed 24/32bpp TGA" << std::endl;
            UnmapViewOfFile(fileData);
            CloseHandle(hTextureFileMapping);
            CloseHandle(hTextureFile);
            return;
    ...
    if (stereoMode) // rozpakowanie do obydwu kontekstów, gdy stereo
    {
        ...
        // Upload texture to card with bound texture ID
        gluBuild2DMipmaps(GL_TEXTURE_2D, internalFormat, m_width, m_height, fileFormat, GL_UNSIGNED_BYTE,
                          &fileData[18]);
